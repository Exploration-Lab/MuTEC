...
def load_and_cache_examples(examples, evaluate=False, no_cache=False, output_examples=False, model_args=None):
    """
    ...
    features, dataset = squad_convert_examples_to_features(
        examples=examples,
        tokenizer=tokenizer,
        max_seq_length=model_args.max_clength,
        doc_stride=512,
        max_query_length=model_args.max_qlength,
...
def get_examples(examples_to_process, is_training=True, version_2_with_negative=True):
    ...
    # paragraph corresponds to one dict containing context, question, answer
    for idx, paragraph in examples_to_process.iterrows():
        context = None
        if 'context' in paragraph:
            context = paragraph['context']
        utterance = paragraph['utterance']
        emotion = paragraph['emotion']
        cause_utterance = paragraph['cause_utterance']
        cause_span = paragraph['cause_span']
        qas_id = paragraph["id"]
        ...
        start_idx = None
        end_idx = None
        unk = config.EMOTION_TOKENIZER.unk_token

        # question = S1 + utterance + " " + S2 + cause_utterance + " " + S3 + unk + ' ?'  
        if context is None:
            question = utterance
            context = cause_utterance
        else:
            cu = '' if cause_utterance is np.nan else cause_utterance
            question = utterance + cu

        if cause_span == "Impossible \u2260\u00e6\u2260\u00e6\u2260 answer":
            is_impossible = True

        if not is_impossible:
            answer_text = cause_span
            ...
            start_position_character = start_idx
        
        # index of emotion if split by whitespace
        emotion_idx = len(question)-2
        ...
        # this is a SQUAD data generator
...
def squad_convert_example_to_features(
    example, max_seq_length, doc_stride, max_query_length, padding_strategy, is_training, model_args
):
    ...
    if is_training and not example.is_impossible:
        ...
        # actual text-> starting to ending word
        # cleaned answer text is the answer span given in the dataset
        # it might be the case that answer span begins in the middle of the word
        # find returns the first occurence of the string in outer string
    ...
    orig_to_tok_index = []
    all_doc_tokens = []
    # orig_to_token -> word in doc_token is at which index in all_doc_tokens
    # all_doc_tokens contain the subwords of the words as well
    # doc tokens contain each word which was separated by whitespace
    # the below code takes in each word and maps similar indices to subtokens of 
    # same word
    for (i, token) in enumerate(example.doc_tokens):
        orig_to_tok_index.append(len(all_doc_tokens))
        # the below tokenizers require a space between each word to
        # tokenize them correctly, that is why add prefix space is 
        # set to True
    ...
    truncated_query = tokenizer.encode(
        example.question_text, add_special_tokens=False, truncation=True, max_length=max_query_length-1
    )
    
    emotion_idx_iids = len(truncated_query)
    # emotion_idx_iids = len(truncated_query) - 1
    ...
    # all docs have tokens for context
    # truncated query have token ids of question
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        ...
        # print(encoded_dict)
        # print(encoded_dict['input_ids'][emotion_idx_iids])
        ...
        # spans will have list of encoded_dict dictionaries 
    ...
    for span in spans:
        ...
        ## Add input_ids, token_type_ids and attention_mask for utterance as well
        utterance = example.utterance
        context = example.context_text
        utterance_encode = config.EMOTION_TOKENIZER.encode_plus(
                            utterance,
                            context if config.FOLD == 'fold1' else None,
                            add_special_tokens=True,
                            max_length=model_args.max_ulength,
                            padding=True,
                            truncation=True,
                        )
        
        utterance_input_ids = utterance_encode['input_ids']
        utterance_token_type_ids = utterance_encode['token_type_ids']
        utterance_mask = utterance_encode['attention_mask']

        padding_ulen = model_args.max_ulength - len(utterance_input_ids)
        if padding_ulen > 0:
            utterance_input_ids = utterance_input_ids + ([tokenizer.pad_token_id]*padding_ulen)
            utterance_mask = utterance_mask + ([tokenizer.pad_token_id]*padding_ulen)
            utterance_token_type_ids = utterance_token_type_ids + ([tokenizer.pad_token_id]*padding_ulen)
        
        emotion=example.emotion
        emotion_idx=example.emotion_idx
...
def squad_convert_examples_to_features(
    ...
    # examples are SquadExample object

    squad_convert_example_to_features_init(tokenizer)
    # squad convert example to features
    annotate_ = partial(
    ...
    # annotate is a partial function, examples is the remaining postional argument
    features = [annotate_(example) for example in tqdm(examples, disable=not tqdm_enabled, position=0, leave=True)]

    # here feature is a SquadFeature object
    # print(features)
    new_features = []
    ...
    max_len = model_args.max_qlength
    ...
    max_ulen = model_args.max_ulength
    all_utterance_input_ids = torch.tensor([f.utterance_input_ids[:max_ulen] for f in features], dtype=torch.long)
    all_utterance_token_type_ids = torch.tensor([f.utterance_token_type_ids[:max_ulen] for f in features], dtype=torch.long)
    all_utterance_mask = torch.tensor([f.utterance_mask[:max_ulen] for f in features], dtype=torch.long)
    all_emotion = torch.tensor([f.emotion for f in features], dtype=torch.long)
    all_emotion_idx = torch.tensor([f.emotion_idx for f in features], dtype=torch.long)
    ...
    if not is_training:
        ...
        dataset = TensorDataset(
            ...
            all_attention_masks, 
            all_token_type_ids,
            # torch.tensor([]),   # start pos
            # torch.tensor([]),   # end pos
        ...
        # TensorDataset internally calls the __getitem__ function
...
def _improve_answer_span(doc_tokens, input_start, input_end, tokenizer, orig_answer_text):
    ...
    # after tokenization join the subtokens with a single space
    # print(tokenizer.tokenize(orig_answer_text))
    tok_answer_text = " ".join(tokenizer.tokenize(orig_answer_text))

    # this code is a 2 pointer approach
    # make left and right pointer and then move the right pointer
    # first close and similarily move the left pointer close 
    # till the text span becomes equal to the answer span
...
class SquadExample:
    ...
    def __init__(
        ...
        # the below for loop is similar to 'self.context_text.split()'
        for c in self.context_text:
            ...
            # char to word offset creates a list of index of each 
            # character belonging to which word
            # [0, 0, 0, 1, 1, 1] for 'He is . . .'

        self.doc_tokens = doc_tokens
        self.char_to_word_offset = char_to_word_offset

        # Start and end positions only has a value during evaluation.
        # this simply returns the start_position word and end_position word
...
class SquadFeatures:
    ...
    def __init__(
        ...
        self.utterance_token_type_ids=utterance_token_type_ids
