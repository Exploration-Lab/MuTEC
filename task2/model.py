...
class ClsModel(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, ut_len=None, ui_len=None):
        ...
        # print(e.size(), c.size(), pool_out.size())
...
class EntailModel:
    ...
    def train_model(self, df_train, df_valid=None):
        ...
        plot_loss(train_loss_list, eval_loss_list, "Cause Emotion Entailment")
    ...
    def eval_fn(self, data_loader, model, device):
        ...
        with torch.no_grad():
            for bi, d in enumerate(tqdm(data_loader, total=len(data_loader), desc="Evaluation")):
                ...
                # loss = self.em_loss_fn(ut_preds, em_targets) \
                # + self.out_loss_fn(final_preds, targets)
                # loss = self.out_loss_fn(final_preds, targets)
                
                # eval_loss.update(loss.item(), targets.size(0))
                
                self.eval_example_cnt += input_ids.size(0)
                self.eval_batch_cnt += 1

                # if ((self.eval_batch_cnt + 1) % 100) == 0:
                #   wandb.log({'eval_loss': loss}, step=self.eval_example_cnt)
