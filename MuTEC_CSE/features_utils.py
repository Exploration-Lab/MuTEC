...
def load_and_cache_examples(examples, evaluate=False, no_cache=False, output_examples=False):
    """
...
def get_examples(examples_to_process, is_training=True, version_2_with_negative=True):
    ...
    for idx, paragraph in examples_to_process.iterrows():
        ...
        start_idx = None
        end_idx = None
        unk = config.CAUSE_TOKENIZER.unk_token
        
        # in question $ signifies emotion
...
def squad_convert_example_to_features(
    ...
    # emotion_idx_iids = len(truncated_query) - 1      # if used as question
    emotion_idx_iids = len(truncated_query)      # if used without question
    ...
    for span in spans:
        ...
        padding_ulen = config.MAX_ULENGTH - len(utterance_input_ids)
        if padding_ulen > 0:
