...
class E2E_Model(nn.Module):
    ...
    def forward(self, 
        ...
        s_final = torch.sigmoid(self.s_linear(start_logits))
        return s_final, e_linear_out
...
class RecModel(nn.Module):
    ...
    def neg_loss(self, neg_logit, neg_val):
        weights = torch.tensor(self.neg_weights, dtype=torch.float)
        weights = weights / weights.sum()
        weights = 1.0 / weights
        weights = weights / weights.sum()
        class_weights = torch.FloatTensor(weights).to(self.device)
        ...
        loss = class_weights[1] * (neg_val.float() * torch.log(neg_logit)) + \
        class_weights[0] * ((1 - neg_val.float()) * torch.log(1 - neg_logit))
        # loss = loss_fn(neg_logit, neg_val.float())
        
        return torch.neg(torch.mean(loss))

    def center_loss(self, center, center_pos):
        loss_fn = nn.MSELoss()
        loss = loss_fn(center, center_pos)
        return loss
    
    def length_loss(self, length, length_pos):
        loss_fn = nn.MSELoss()
        loss = loss_fn(length, length_pos)
        return loss
    ...
    def train(self, data_loader, model, optimizer, device, scheduler):
        ...
        for bi, d in enumerate(tk0):
            ...
            neg_loss = self.neg_loss(s_final[:, 0], (start_positions!=0).long())
            mse_center = self.center_loss(s_final[:, 1], torch.true_divide(((start_positions+end_positions)//2), 512))
            mse_length = self.length_loss(s_final[:, 2], torch.true_divide((end_positions-start_positions), 512))
            
            e_loss = self.e_loss_fn(e_linear_out, emotion)
            mse_center = torch.sum(mse_center*(start_positions!=0))
            mse_length = torch.sum(mse_length*(start_positions!=0))
            
            loss = neg_loss + mse_center + mse_length + e_loss
            ...
            if config.WANDB:
                if ((self.batch_cnt + 1) % 100) == 0:
                    ...
                    'mse_center_loss': mse_center, 'mse_length_loss': mse_length, 'neg_loss': neg_loss})
    ...
    def eval(self, data_loader, examples, features, model, device):
        model.eval() 
        RawResult = collections.namedtuple("RawResult", ["unique_id", "neg_val", "center", "length"])  
        all_results = []
        eval_loss = 0.0
        nb_eval_steps = 0
        e_targets = []
        ...
        with torch.no_grad():
            ...
            for bi, d in enumerate(tk0):
                ...
                neg_val = s_final[:, 0].detach().cpu().numpy()
                center = s_final[:, 1].detach().cpu().numpy()
                length = s_final[:, 2].detach().cpu().numpy()
                ...
                neg_targets.extend((start_positions!=0).long().cpu().detach().numpy().tolist())
                neg_outputs.extend(neg_val)
                e_targets.extend(emotion.cpu().detach().numpy().tolist())
                ...
                # losses.update(loss.item(), input_ids.size(0))
                # tk0.set_postfix(loss=losses.avg)
        all_predictions = self.write_predictions_new(
        ...
        neg_outputs = np.array(neg_outputs) >= 0.5 
        neg_acc = metrics.accuracy_score(neg_targets, neg_outputs)
        report = metrics.classification_report(neg_targets, neg_outputs)
        print(neg_acc)
        print(report)
    ...
    def write_predictions_new(
        ...
        for (example_index, example) in enumerate(all_examples):
            ...
            for (feature_index, feature) in enumerate(features):
                ...
                if result.neg_val < 0.5:
                    final_text = ''
                    
                start_index = int((result.center - result.length/2)*512)
                end_index = int((result.center + result.length/2)*512)
