...
class E2E_Model(nn.Module):
    ...
    def forward(self, 
        ...
        neg_logits = self.neg_linear(start_logits)
        # start_logit_wn = torch.hstack([start_logits, (start_positions==0).unsqueeze(1)])
        ...
        if start_positions is not None and end_positions is not None: #training
            ...
            return start_logits, end_logits, e_linear_out, neg_logits
            ...
            neg_val = torch.argmax(F.softmax(neg_logits, dim=-1), dim=-1)
            # outputs = start_top_log_probs, start_top_index, end_top_log_probs, end_top_index 
            return start_top_log_probs, start_top_index, end_top_log_probs, end_top_index, e_linear_out, neg_val
...
class RecModel(nn.Module):
    ...
    def neg_loss_fn(self, neg_logit, neg_val):
        ...
        class_weights = torch.FloatTensor(weights).to(self.device)
        loss_fn = nn.CrossEntropyLoss(weight=class_weights)
        loss = loss_fn(neg_logit, neg_val.long())
        return loss
    ...
    def train(self, data_loader, model, optimizer, device, scheduler):
        ...
        for bi, d in enumerate(tk0):
            ...
            start_logits, end_logits, e_linear_out, neg_logits = model(
                input_ids=input_ids, 
                token_type_ids=token_type_ids, 
                attention_mask=mask, 
            ...
            neg_loss = self.neg_loss_fn(neg_logits, start_positions==0)
            
            loss = c_loss + config.BETA * e_loss + neg_loss
            loss.backward()
    ...
    def eval(self, data_loader, examples, features, model, device):
        model.eval() 
        RawResult = collections.namedtuple("RawResult", ["unique_id", "start_top_log_probs", "start_top_index", "end_top_log_probs", "end_top_index", "neg_val"])  
        all_results = []
        eval_loss = 0.0
        nb_eval_steps = 0
        e_targets = []
        ...
        with torch.no_grad():
            ...
            for bi, d in enumerate(tk0):
                ...
                start_top_log_probs, start_top_index, end_top_log_probs, end_top_index, e_linear_out, neg_val = model(
                    input_ids=input_ids, 
                    token_type_ids=token_type_ids, 
                    attention_mask=mask, 
                    utterance_input_ids=utterance_input_ids,
                ...
                neg_val = neg_val.detach().cpu().numpy()
